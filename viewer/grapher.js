...
grapher.Edge = class {
    ...
    build(document, edgePathGroupElement, edgePathHitTestGroupElement, edgeLabelGroupElement) {
        ...
        let datatype = this.value.value && this.value.value.type && this.value.value.type.dataType ? this.value.value.type.dataType : '';
        if (['uint8', 'uint16', 'uint32', 'uint64'].includes(datatype)) {
            datatype = '-int';
        } else if (['int8', 'int16', 'int32', 'int64'].includes(datatype)) {
            datatype = '-int';
        } else if (['float8', 'float16', 'float32', 'double'].includes(datatype)) {
            datatype = '-float';
        }

        this.element.setAttribute('class', this.class ? `edge-path ${this.class}` : `edge-path${datatype}`);
