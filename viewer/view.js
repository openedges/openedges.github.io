    ...
    render() {
        ...
        if (tensor) {
            ...
            if (type) {
                ...
                const shapeStr = shape === '' ? '[]' : shape; // if scalar case, show [] for shape
                this.addProperty('shape', `${shapeStr}`, 'code');
...
view.ModelFactoryService = class {
    ...
    constructor(host) {
        ...
        this.register('./enlight', ['.enlight']);
        this.register('./enlight_pb', ['.enlight2']);
        /* eslint-enable no-control-regex */
    }
    ...
    async import() {
        if (this._host.type === 'Browser' || this._host.type === 'Python') {
            const files = [
                ...
                './enlight', './enlight-metadata.json', './enlight-schema', './enlight_pb'
            ];
