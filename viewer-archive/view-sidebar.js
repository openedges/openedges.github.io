...
sidebar.NodeSidebar = class {
    ...
    constructor(host, node) {
        ...
        let divider = this._host.document.createElement('div');
    ...
    _addAttribute(name, attribute) {
        let item = new sidebar.NameValueView(this._host, name, new NodeAttributeView(this._host, attribute));
    ...
    _addInput(name, input) {
        if (input.arguments.length > 0) {
            let view = new sidebar.ParameterView(this._host, input);
    ...
    static formatAttributeValue(value, type, quote) {
        ...
        let list = [];
        let keys = Object.keys(value).filter((key) => !key.startsWith('__') && !key.endsWith('__'));
        if (keys.length == 1) {
...
sidebar.NameValueView = class {
    ...
    constructor(host, name, value) {
        ...
        let nameElement = this._host.document.createElement('div');
        nameElement.className = 'sidebar-view-item-name';

        let nameInputElement = this._host.document.createElement('input');
...
sidebar.SelectView = class {
    ...
    constructor(host, values, selected) {
        ...
        let selectElement = this._host.document.createElement('select');
...
sidebar.ValueTextView = class {
    ...
    constructor(host, value, action) {
        this._host = host;
        this._elements = [];
        let element = this._host.document.createElement('div');
        ...
        let line = this._host.document.createElement('div');
...
class NodeAttributeView {
    ...
    toggle() {
        if (this._expander.innerText == '+') {
            ...
            let description = this._attribute.description;
            if (description) {
...
sidebar.ToogleView = class {

    constructor(host, value, value_info) {
        this._host = host;
        this._argument = value_info;
        ...
        let idLine = this._host.document.createElement('div');
        idLine.className = 'sidebar-view-item-value-line';
        idLine.innerHTML = '<span class=\'sidebar-view-item-value-line-content\'>' + value + '</span>';
    ...
    toggle() {
        if (this._expander) {
            if (this._expander.innerText == '+') {
                ...
                let value = this._argument;
                if (value) {
                    let Line = this._host.document.createElement('div');
                    Line.className = 'sidebar-view-item-value-line-border';
                    Line.innerHTML = '<span class=\'sidebar-view-item-value-line-content\'>' + value + '</span>';
...
sidebar.NameToggleView = class {
    ...
    constructor(host, name, value) {
        ...
        valueElement.appendChild(value.render());
...
sidebar.ArgumentView = class {
    ...
    constructor(host, argument) {
        ...
        let initializer = argument.initializer;
        if (initializer) {
            this._element.classList.add('sidebar-view-item-value-dark');
        }

        let quantization = argument.quantization;
...
sidebar.ModelSidebar = class {
    ...
    constructor(host, model, graph) {
        ...
        if (this._model.format == 'EnlightNN'){
            this._addProperty('model', new sidebar.ValueTextView(this._host, this._model.model));

            let model_type;
            if (this._model.model_type == 'class'){
                model_type = 'classification'
            }
            else if (this._model.model_type == 'obj'){
                model_type = 'object detection';
            }
            else {
                model_type = this._model.model_type;
            }

            this._addProperty('model_type', new sidebar.ValueTextView(this._host, model_type));
            this._addTestView('num_class', this._model.num_class, this._model.class_labels);
            this._addProperty('has_detection_layer', new sidebar.ValueTextView(this._host, this._model.has_detection_layer));

            this._addHeader('Evaluation');

            let has_score = this._model.has_score;
            if (has_score){
                if (this._model.model_type == 'class'){
                    if (this._model.top5 != null){
                        let top5 = (this._model.top5).toFixed(2);
                        this._addProperty('top5', new sidebar.ValueTextView(this._host, top5));
                    }
                    else {
                        let top1 = (this._model.top1).toFixed(2);
                        this._addProperty('top1', new sidebar.ValueTextView(this._host, top1));
                    }
                } else {
                    let mAP = (this._model.mAP * 100).toFixed(2);
                    this._addProperty('mAP', new sidebar.ValueTextView(this._host, mAP));
                }

                this._addProperty('dataset', new sidebar.ValueTextView(this._host, this._model.evaluation_dataset));
            }
            else
                this._addProperty('has_score', new sidebar.ValueTextView(this._host, has_score));

            this._addHeader('Track statistics');


            this._addProperty('is_tracked', new sidebar.ValueTextView(this._host, this._model.is_tracked));
            this._addProperty('has_histogram', new sidebar.ValueTextView(this._host, this._model.has_histogram));

            let is_tracked = this._model.is_tracked | this._model.has_histogram;
            if (is_tracked){
                this._addProperty('dataset', new sidebar.ValueTextView(this._host, this._model.track_dataset));
                this._addProperty('num_images', new sidebar.ValueTextView(this._host, this._model.num_images));
            }

            this._addHeader('Input normalization');
            this._addProperty('is_fused', new sidebar.ValueTextView(this._host, this._model.is_fused_normalization));

            if (this._model.is_fused_normalization) {
                let means = [];
                let stds = [];

                for (let mean of this._model.norm_mean){
                    means.push(mean.toFixed(3));
                }
                for (let std of this._model.norm_std){
                    stds.push(std.toFixed(3));
                }

                this._addProperty('mean', new sidebar.ValueTextView(this._host, '['+means+']'));
                this._addProperty('std', new sidebar.ValueTextView(this._host, '['+stds+']'));
            }



            if (this._model.optimizations) {
                this._addHeader('Optimizations');

                let cnt = 0;
                let opts = this._model.optimizations
                for (let opt of opts) {
                    this._addProperty('#' + cnt, new sidebar.ValueTextView(this._host, opt));
            ...
            let is_quantized = this._model.is_quantized;
            if (is_quantized) {
                this._addProperty('method', new sidebar.ValueTextView(this._host, this._model.quantization_method));
                this._addProperty('m_std_8', new sidebar.ValueTextView(this._host, this._model.m_std_8));
                this._addProperty('m_std_4', new sidebar.ValueTextView(this._host, this._model.m_std_4));
                this._addProperty('m_std_ratio', new sidebar.ValueTextView(this._host, this._model.m_std_ratio.toFixed(3)));
                this._addProperty('clip_min_max', new sidebar.ValueTextView(this._host, this._model.clip_min_max));
                this._addProperty('iter_weight_mean_correction', new sidebar.ValueTextView(this._host, this._model.iter_weight_mean_correction));
                this._addProperty('quantize_post_process', new sidebar.ValueTextView(this._host, this._model.quantize_post_process));
            }
            else
                this._addProperty('is_quantized', new sidebar.ValueTextView(this._host, is_quantized));
        }
    ...
    _addTestView(name, value, value_info) {
        let view = new sidebar.ToogleView(this._host, value, value_info);
        let item = new sidebar.NameToggleView(this._host, name, view);
        this._elements.push(item.render());
    }
